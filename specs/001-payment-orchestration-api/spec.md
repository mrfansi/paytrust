...
- How does the system handle webhook failures or delayed webhook notifications from payment gateways? System retries webhook processing 3 times with fixed intervals at cumulative delays from initial failure (T=0): retry 1 at T+1 minute, retry 2 at T+6 minutes, retry 3 at T+36 minutes per FR-042
...
- How does the system handle refunds or payment reversals from the gateway? System tracks refund status via webhooks per FR-086, provides GET /invoices/{id}/refunds endpoint for refund history. Refund initiation handled directly with payment gateways per Assumptions L340
- What happens when timezone differences cause payment timestamp discrepancies? System stores all timestamps in UTC internally per FR-087, converts to gateway-required timezone for API calls (Xendit: UTC, Midtrans: Asia/Jakarta UTC+7), returns ISO 8601 UTC timestamps in API responses
...
- **FR-073**: System MUST handle installment overpayments (payment amount exceeds installment amount) with the following behavior: (a) accept overpayment on individual installments, (b) automatically apply excess payment amount sequentially to remaining unpaid installments in order, (c) mark installments as "paid" when covered by excess payment application, (d) mark entire invoice as "fully_paid" if total payment received equals or exceeds invoice total regardless of installment distribution, (e) if overpayment exceeds total invoice amount store excess in payment_transactions.overpayment_amount field (DECIMAL(19,4)) and provide GET /invoices/{id}/overpayment endpoint returning {invoice_id, total_amount, total_paid, overpayment_amount} for merchant reference. PayTrust stores overpayment_amount but does NOT initiate refunds. Merchant must process refund through gateway dashboard per Assumptions section refund workflow. PayTrust tracks refund status via webhook per FR-086
...
- **FR-042**: System MUST retry failed webhook processing with audit logging, implementing the following behavior:
...
- Refund initiation is handled directly with payment gateways (outside PayTrust API), but PayTrust tracks refund status via gateway webhooks per FR-086 and provides GET /invoices/{id}/refunds endpoint for refund history queries. Refund workflow: (1) Merchant identifies overpayment via GET /invoices/{id}/overpayment endpoint, (2) Merchant initiates refund through gateway dashboard (Xendit/Midtrans admin panel) using gateway transaction ID, (3) Gateway processes refund and sends webhook notification to PayTrust, (4) PayTrust updates transaction records with refund details (refund_id, refund_amount, refund_timestamp, refund_reason), (5) Merchant queries refund status via GET /invoices/{id}/refunds for reconciliation
