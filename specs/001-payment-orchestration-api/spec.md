...
- **FR-042**: System MUST retry failed webhook processing 3 times using fixed intervals at cumulative delays from initial failure (T=0): retry 1 at T+1 minute (1 minute after initial failure), retry 2 at T+6 minutes (6 minutes after initial failure, 5 minutes after retry 1), retry 3 at T+36 minutes (36 minutes after initial failure, 30 minutes after retry 2). Failed webhook defined as: HTTP 5xx response or connection timeout >10s. Retry only for 5xx errors and timeouts. 4xx errors (validation failures including signature verification failures) marked permanently failed immediately without retry. Retry timers are in-memory only and do NOT persist across application restarts - pending retries are lost on restart and require manual reprocessing via GET /webhooks/failed endpoint. After all 3 retries fail, mark webhook as permanently failed, log error with CRITICAL level, and expose failed webhooks via GET /webhooks/failed endpoint for manual intervention
...
- **FR-085**: System MUST set payment_initiated_at timestamp on Invoice entity when first payment attempt is made, defined as: gateway payment URL generation OR payment_transaction record creation, whichever occurs first. Use this timestamp to enforce invoice immutability per FR-051 (reject modifications when payment_initiated_at IS NOT NULL)
