...
- **FR-083**: System MUST provide API endpoints for generating, rotating, and revoking API keys with audit logging (POST /api-keys, PUT /api-keys/{id}/rotate, DELETE /api-keys/{id}), using argon2 hashing algorithm for secure key storage. Audit log MUST include: timestamp (UTC), operation type (created/rotated/revoked/used), api_key_id, actor identifier, IP address, old key hash (for rotation only), success/failure status
