...
- **FR-042**: System MUST retry failed webhook processing with the following behavior:
...
- **FR-083**: System MUST provide API endpoints for generating, rotating, and revoking API keys with audit logging (POST /api-keys, PUT /api-keys/{id}/rotate, DELETE /api-keys/{id}), using argon2 hashing algorithm for secure key storage. Audit log MUST include: timestamp (UTC), operation type (created/rotated/revoked/used), api_key_id, actor identifier, IP address, old key hash (for rotation only), success/failure status. Audit logs retained per NFR-011 (1 year minimum)
- **FR-083a**: System MUST provide GET /api-keys/audit endpoint with pagination (page, page_size parameters) and date range filtering (start_date, end_date parameters in ISO 8601 format) to query API key audit logs for security monitoring and compliance
