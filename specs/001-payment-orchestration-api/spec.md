...
- **FR-086**: System MUST track refund status via gateway webhooks and update transaction records with refund information (refund_id, refund_amount, refund_timestamp, refund_reason). Provide GET /invoices/{id}/refunds endpoint returning refund history. Refund initiation handled directly with payment gateways per Assumptions L340
