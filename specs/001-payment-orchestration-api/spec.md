...
- **FR-082**: System MUST provide API to create supplementary invoices that reference the original invoice for additional items requested mid-payment-cycle, with validation: (a) supplementary invoice MUST reference valid parent invoice_id, (b) parent invoice MUST exist and be in status: pending, partially_paid, or paid (reject if parent is expired, cancelled, or failed with 400 Bad Request "Cannot create supplementary invoice for {status} parent invoice"), (c) parent invoice with status=draft is rejected because draft means payment_initiated_at IS NULL per FR-051(a), indicating no active payment flow exists, (d) parent invoice MUST NOT itself be a supplementary invoice (prevent chaining, reject with 400 Bad Request "Cannot create supplementary invoice from another supplementary invoice"), (e) inherit currency and gateway from parent invoice, (f) maintain separate payment schedule with independent installment configuration (supplementary invoice can have different installment count and amounts than parent), (g) supplementary invoice expiration is independent of parent - if parent invoice expires after supplementary is created, supplementary invoice lifecycle continues independently and can be paid/expired according to its own expires_at timestamp regardless of parent status changes
- **FR-044**: System MUST manage invoice expiration with the following requirements: (a) Default expiration: 24 hours from creation timestamp if expires_at parameter not provided, (b) Optional expires_at parameter: Accept ISO 8601 timestamp in invoice creation request for custom expiration, (c) Validation - Past dates: Reject requests with expires_at in the past with 400 Bad Request "Expiration time cannot be in the past", (d) Validation - Minimum: Reject expires_at < created_at + 1 hour with 400 Bad Request "Expiration must be at least 1 hour from now", (e) Validation - Maximum: Reject expires_at > created_at + 30 days with 400 Bad Request "Expiration must be within 30 days from now", (f) Validation - Installments: If invoice has installments, expires_at MUST be >= last installment due_date (reject with 400 Bad Request "Invoice expiration cannot occur before final installment due date {due_date}" if violated)
...
- **FR-042**: System MUST retry failed webhook processing with the following behavior:
  ...
  **Retry Persistence**:
  ...
  - Pending retries lost on restart, require manual reprocessing via GET /webhooks/failed endpoint
