...
async fn test_gateway_currency_validation_at_payment_time() {
    ...
    assert!(true, "Currency validation should occur at payment initiation");
}
...
fn test_gateway_currency_matrix() {
    ...
    assert!(xendit_currencies.contains(&"IDR"), "Xendit must support IDR");
    assert!(xendit_currencies.contains(&"MYR"), "Xendit must support MYR");
    
    // Midtrans supported currencies
    let midtrans_currencies = vec!["IDR"];
    assert!(midtrans_currencies.contains(&"IDR"), "Midtrans must support IDR");
    assert!(!midtrans_currencies.contains(&"MYR"), "Midtrans must NOT support MYR");
    assert!(!midtrans_currencies.contains(&"SGD"), "Midtrans must NOT support SGD");
}
...
fn test_currency_codes_are_valid_iso() {
    ...
    for currency in all_supported {
        assert_eq!(currency.len(), 3, "Currency '{}' must be 3-letter ISO code", currency);
