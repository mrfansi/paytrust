...
fn test_expiration_duration_calculation() {
    ...
    assert!(check_time_before < expiry_time, "Invoice should not be expired before 24 hours");
    
    // Test at expiration
    let check_time_at = created_at + Duration::hours(24);
    assert!(check_time_at >= expiry_time, "Invoice should be expired at 24 hours");
    
    // Test after expiration
    let check_time_after = created_at + Duration::hours(25);
    assert!(check_time_after >= expiry_time, "Invoice should be expired after 24 hours");
}
...
fn test_expiration_prevents_payment_initiation() {
    ...
    assert!(invoice_statuses.contains(&"pending"), "Pending status must exist");
    assert!(invoice_statuses.contains(&"expired"), "Expired status must exist");
    
    // Expired invoices should not allow payment
    let can_pay_expired = false;
    assert!(!can_pay_expired, "Expired invoices must not allow payment initiation");
}
...
fn test_expiration_timing_constants() {
    ...
    assert_eq!(expiration_hours, 24, "Expiration period must be 24 hours per FR-044");
