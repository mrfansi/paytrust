...
async fn test_sequential_installment_payment_enforcement() -> Result<()> {
    ...
    let result = transaction_service
        .process_installment_payment(
            invoice_id.clone(),
            schedules[1].id.unwrap(), // Installment #2
            Decimal::from_str("100.00").unwrap(),
            "gateway-tx-001".to_string(),
        )
        .await;
    
    assert!(result.is_err(), "Should reject payment of installment #2 before #1 (FR-068)");
    
    // Pay installment #1 (should succeed)
    let result = transaction_service
        .process_installment_payment(
            invoice_id.clone(),
            schedules[0].id.unwrap(), // Installment #1
            Decimal::from_str("100.00").unwrap(),
    ...
    assert!(result.is_ok(), "Should allow payment of installment #2 after #1");
...
async fn test_cannot_skip_installments() -> Result<()> {
    ...
    let result = transaction_service
        .process_installment_payment(
            invoice_id.clone(),
            schedules[2].id.unwrap(), // Installment #3
            Decimal::from_str("100.00").unwrap(),
            "gateway-tx-005".to_string(),
        )
        .await;
    
    assert!(result.is_err(), "Should reject payment of installment #3 before #2 (FR-069)");
    
    // Attempt to pay installment #4 (skipping #2 and #3) - should fail
    let result = transaction_service
        .process_installment_payment(
            invoice_id.clone(),
            schedules[3].id.unwrap(), // Installment #4
            Decimal::from_str("100.00").unwrap(),
            "gateway-tx-006".to_string(),
        )
        .await;
    
    assert!(result.is_err(), "Should reject payment of installment #4 before #2 and #3 (FR-069)");
...
async fn test_partial_payment_allows_next_installment() -> Result<()> {
    ...
    assert!(result.is_err(), "Should reject next installment until current is fully paid");
    ...
    assert!(result.is_ok(), "Should allow next installment after completing previous");
