...
use paytrust::modules::{
    ...
    invoices::{models::LineItem, repositories::InvoiceRepository, services::InvoiceService},
...
async fn test_cannot_adjust_paid_installments() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Should reject adjustment of paid installment (FR-078)"
    );
...
async fn test_adjusted_amounts_must_match_remaining_balance() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Should accept adjustment when sum matches remaining balance"
    );

    // Verify adjustments applied
    let updated_schedules = installment_repo.find_by_invoice(&invoice_id).await?;
    let remaining_total: Decimal = updated_schedules[1..].iter().map(|s| s.amount).sum();
...
async fn test_adjust_all_unpaid_installments_simultaneously() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Should allow simultaneous adjustment of all unpaid installments"
    );
    ...
    assert_eq!(
        updated_schedules[3].amount,
        Decimal::from_str("75.00").unwrap()
    );
    assert_eq!(
        updated_schedules[4].amount,
        Decimal::from_str("125.00").unwrap()
    );
    assert_eq!(
        updated_schedules[5].amount,
...
async fn test_adjustment_maintains_proportional_tax_and_fees() -> Result<()> {
    ...
    let line_items = vec![LineItem::new_with_tax(
        "Test Product".to_string(),
        1,
    ...
    let remaining_tax: Decimal = updated_schedules[1..].iter().map(|s| s.tax_amount).sum();
    let remaining_fee: Decimal = updated_schedules[1..]
        .iter()
    ...
    assert_eq!(
        remaining_tax, original_remaining_tax,
        "Tax should be redistributed proportionally"
    );
    assert_eq!(
        remaining_fee, original_remaining_fee,
        "Service fee should be redistributed proportionally"
    );

    Ok(())
}

async fn test_adjust_unpaid_installments_before_first_payment() -> Result<()> {
    let pool = create_test_pool().await;
    ...
    let result = installment_service
        .adjust_unpaid_installments(&invoice_id, adjustments)
        .await;
