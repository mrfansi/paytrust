...
use paytrust::modules::installments::controllers::{get_installments, adjust_installments};
...
async fn test_get_installments_contract() {
    ...
    if resp.status().is_success() {
        ...
        assert!(body.get("installments").is_some(), "Response must have installments array");
...
async fn test_get_installments_no_schedule() {
    ...
    if resp.status().is_success() {
        let body: Value = test::read_body_json(resp).await;
        let installments = body["installments"].as_array().unwrap();
        assert_eq!(installments.len(), 0, "Should return empty array for invoice without installments");
    }
}
...
async fn test_patch_installments_contract() {
    ...
    if resp.status().is_success() {
        ...
        let installments = body["installments"].as_array().unwrap();
        assert!(!installments.is_empty(), "Should return updated installments");
    }
}
...
async fn test_get_installments_includes_breakdown() {
    ...
    if resp.status().is_success() {
        ...
        if !installments.is_empty() {
            ...
            assert!(fee_amount.is_ok(), "Service fee amount must be valid decimal");
        }
    }
}
