...
fn test_proportional_tax_distribution() {
    ...
    let calculated_tax_total: Decimal = installments.iter()
        .map(|i| i.tax_amount)
        .sum();
    
    assert_eq!(calculated_tax_total, total_tax, "Tax amounts must sum to total tax");
}
...
fn test_proportional_service_fee_distribution() {
    ...
    let calculated_fee_total: Decimal = installments.iter()
        .map(|i| i.service_fee_amount)
        .sum();
    
    assert_eq!(calculated_fee_total, total_service_fee, "Service fees must sum to total fee");
}
...
fn test_last_installment_absorbs_rounding() {
    ...
    assert_eq!(calculated_total, total_amount, "Last installment must absorb rounding difference");
    
    // Verify first two are equal
    assert_eq!(installments[0].amount, installments[1].amount, "First installments should be equal");
    
    // Last installment may differ
    assert!(installments[2].amount >= Decimal::ZERO, "Last installment must be non-negative");
}
