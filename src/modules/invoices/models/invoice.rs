...
impl Invoice {
    ...
    fn validate_external_id(external_id: &str) -> Result<()> {
        ...
        if external_id.len() > 100 {
            return Err(AppError::validation("External ID cannot exceed 100 characters"));
        }
    ...
    fn validate_line_items(line_items: &[LineItem]) -> Result<()> {
        if line_items.is_empty() {
            return Err(AppError::validation("Invoice must have at least one line item"));
        }
        
        Ok(())
    }
    
    fn validate_line_items_currency(line_items: &[LineItem], invoice_currency: Currency) -> Result<()> {
...
mod tests {
    ...
    fn create_test_line_item(description: &str, quantity: i32, price: i32, currency: Currency) -> LineItem {
        LineItem::new(
    ...
    fn test_invoice_validation_empty_line_items() {
        ...
        assert!(result.unwrap_err().to_string().contains("at least one line item"));
    }
    ...
    fn test_invoice_validation_currency_mismatch() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("does not match invoice currency"));
    }
